...
for a = 0.1 : 0.1 : 2
    ...
    for i = 1 : 1 : 512/8
        for j = 1 : 1 : 512/8
            temp9{i, j} = round(temp{i, j}./(a*Cannon)).*(a*Cannon);
        end
    end
    %Applying 2D IDCT and calculate the PSNR
    for i = 1 : 1 : 512/8
        for j = 1 : 1 : 512/8
            temp10{i, j} = idct2(temp9{i, j});
            psnr_cannon1(i, j) = PSNR(blocks{i, j}, temp10{i, j});
        end
    end
    filename1 = strcat('F:\Programming\多媒体\hw\Big1\2011011313_exp1\exp2_data_res\Cannon_Q_block_psnr', num2str(a));
    filename1_txt = strcat(filename1, '.txt');
    save(filename1_txt, 'psnr_cannon1', '-ascii', '-tabs');
    cur_mat = cell2mat(temp10);
    psnr_arr_cannon(cnt) = PSNR(gray, cur_mat);
    filename2 = strcat('F:\Programming\多媒体\hw\Big1\2011011313_exp1\res_pics\exp2\as\Cannon_Q_block_psnr', num2str(a));
    filename_img = strcat(filename2, '.bmp');
    imwrite(mat2gray(cur_mat), filename_img);
    cnt = cnt + 1;
end
...
for a = 0.1 : 0.1 : 2
    ...
    for i = 1 : 1 : 512/8
        for j = 1 : 1 : 512/8
            temp11{i, j} = round(temp{i, j}./(a*Nikon)).*(a*Nikon);
        end
    end
    %Applying 2D IDCT and calculate the PSNR
    for i = 1 : 1 : 512/8
        for j = 1 : 1 : 512/8
            temp12{i, j} = idct2(temp11{i, j});
            psnr_nikon1(i, j) = PSNR(blocks{i, j}, temp12{i, j});
        end
    end
    filename1 = strcat('F:\Programming\多媒体\hw\Big1\2011011313_exp1\exp2_data_res\Nikon_Q_block_psnr', num2str(a));
    filename1_txt = strcat(filename1, '.txt');
    save(filename1_txt, 'psnr_nikon1', '-ascii', '-tabs');
    cur_mat = cell2mat(temp12);
    psnr_arr_nikon(cnt) = PSNR(gray, cur_mat);
    filename2 = strcat('F:\Programming\多媒体\hw\Big1\2011011313_exp1\res_pics\exp2\as\Nikon_Q_block_psnr', num2str(a));
    filename_img = strcat(filename2, '.bmp');
    imwrite(mat2gray(cur_mat), filename_img);
    cnt = cnt + 1;
end
